...
var adjust_diagnosis = (diagnosis, diagObj) => {
    ...
    if (diagObjV2.children == []) delete diagObjV2.children;

    for (var bKey in diagObjV2) {
        if(diagObjV2.hasOwnProperty(bKey)) {

            if(diagObjV2[bKey] === ''){
                delete diagObjV2[bKey];
            }
        }
    }
...
function do_interface(){
    ...
    if(yorn === 'y')
        write_obj_to_JSON(construct_indexV2(indexed_data), 'indexed_diagnoses_empty_keys_removed');

    var indexed_dataV2 = JSON.parse(fs.readFileSync('./JSONs/indexed_diagnoses_empty_keys_removed.json', 'utf8'));
